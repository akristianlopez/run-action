...
export const ActModeler = {
    ...
    renderToHTML(elements, userData = {}) {
        ...
        // 2. Rendu du corps (ton code existant avec le wrapper de sélection)
        fullHtml += elements.map(el => {
            const isSelected = this.state.selectedId === el.id;
            let content = '';

            switch(el.type) {
                case 'admin_acte': content = this.renderAdminActe(el.props); break;
                case 'table': content = this.renderTable(el.props); break;
                case 'chart': content = this.renderChart(el.props); break;
                case 'action_word': 
                    content = `<div class="text-center my-6 font-bold underline uppercase text-xl">${el.props.value} :</div>`; 
                    break;
                default: content = `<div class="p-2 border border-dashed text-slate-400">Élément inconnu</div>`;
            }

            // Wrapper avec gestion du clic et style de sélection
            return `
                <div onclick="window.ActModeler.selectElement('${el.id}')" 
                     class="relative cursor-pointer transition-all mb-4 ${isSelected ? 'ring-2 ring-indigo-500 rounded-lg shadow-md' : 'hover:ring-1 hover:ring-slate-300'}">
                    ${content}
                    ${isSelected ? '<div class="absolute -top-3 -right-3 bg-indigo-500 text-white text-[10px] px-2 py-1 rounded-full shadow-lg">Sélectionné</div>' : ''}
                </div>`;
        }).join('');

        // 3. Rendu du pied de page
    ...
    getDefaultProps(type) {
        switch(type) {
            case 'admin_acte': return { documentType: 'ARRETE', decideWord: 'DECIDE', vus: [], considerants: [], documentTitle: 'Portant nomination de...' };
            case 'table': return { rows: 3, cols: 2, data: [['H1', 'H2'], ['', ''], ['', '']], hasHeader: true };
            case 'chart': return { id: Date.now(), chartType: 'bar', title: 'Stats', labels: ['Q1', 'Q2'], data: [10, 20] };
            case 'action_word': return { value: 'DECIDE' };
            default: return {};
        }
    },
        ...
        if (el.type === 'admin_acte') {
            ...
            for (let r = 0; r < el.props.rows; r++) {
                ...
                for (let c = 0; c < el.props.cols; c++) {
                    tableEditor += `
                        <input type="text" value="${el.props.data[r+1][c+1] || ''}" 
    ...
    uDeep(k, v) {
        ...
        if (el) {
            el.props[k] = v;
            this.render();
            // On ne rappelle PAS updateInspector ici pour ne pas perdre le focus de l'input pendant la saisie
        }
    },
    renderHeader() {
